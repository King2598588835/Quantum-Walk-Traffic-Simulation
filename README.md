...
### Step 8: 基于生成的网络密度提取路网

* ​**功能**​：​**通过上一步生成的网络密度手动对路网进行提取**​。
  
  * ​**热力图预处理**​：在arcgis上对 Step 7 生成的网格密度进行对数标准化。
  * ​**ARCGIS要素转换**​：通过 **ARCGIS文件转点要素功能** 将上一步生成的网格密度数值转换为点要素，并进行后续处理。
  * ​**矢量化与简化**​：利用ARCGIS绘制通过点要素绘制道路网络的基本骨架，并进行简化处理，生成道路网络骨架。连接不相连的道路网络，并进行相应的合理化处理，最后保存为 .shp 文件。
#### 🗺️ 手动提取效果展示
...
### 注：Step 11（第1步之后可直接运行下面代码进行一步化处理，中间需要经过道路网络的手动提取）
